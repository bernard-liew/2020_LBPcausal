...
early_diff <- wk10.df - base.df
late_diff <- wk52.df - base.df


names(early_diff) <- paste0(str_remove(names (early_diff), "wk10_"), "_early")
names(late_diff) <- paste0(str_remove(names (late_diff), "wk52_"), "_late")
...
```{r}
tableby (subgrp ~., data = early.df , digits = 2, digits.p = 2) %>%
  summary (text = TRUE) 

tableby (grp ~., data = early.df , digits = 2, digits.p = 2) %>%
  summary (text = TRUE) 

tableby (subgrp ~., data = late.df , digits = 2, digits.p = 2) %>%
  summary (text = TRUE) 

tableby (grp ~., data = late.df , digits = 2, digits.p = 2) %>%
  summary (text = TRUE)
...
```{r}
 aggr(early.df , col=c('navyblue','yellow'),
            numbers=TRUE, sortVars=TRUE,
            labels=names(early.df), cex.axis=.7,
            gap=3, ylab=c("Missing data","Pattern"))

 aggr(late.df , col=c('navyblue','yellow'),
            numbers=TRUE, sortVars=TRUE,
            labels=names(late.df), cex.axis=.7,
            gap=3, ylab=c("Missing data","Pattern"))
...
saveRDS (early.df, "output/df_early.RDS")
saveRDS (late.df, "output/df_late.RDS")
