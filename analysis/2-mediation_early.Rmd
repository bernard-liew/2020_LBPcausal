...
This is a bayesian network analysis where the variables are the change scores between week 10 and baseline.
...
```{r}
df <- df_list[["wk10_base"]]
names(df)[1:8] <- paste0(str_remove(names(df)[1:8] , "wk10_"), "_early")
```
...
```{r}
wl =  matrix(c("grp", "lbp_early"), nrow = 1, ncol = 2, byrow = TRUE, 
