...
df3 <- df_list[["wk52"]]
df3 <- df3 %>%
  select (id, grp, subgrp, wk52_lbp, wk52_lp, wk52_func) 
...
df.bn[, grepl ( "id|subgrp|osw|outcome",names (df.bn))] <- NULL
...
df.bn = as.data.frame (df)
df.bn[, grepl ( "id|subgrp|osw",names (df.bn))] <- NULL
